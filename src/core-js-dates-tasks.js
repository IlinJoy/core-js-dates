...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 0; i < totalDays; i += 1) {
    ...
    if (currentDay === 0 || currentDay === 6) weekends += 1;
  }
...
function getNextFridayThe13th(date) {
  const dateForFind = new Date(date);
  dateForFind.setUTCDate(13);

  while (dateForFind.getUTCDay() !== 5) {
    dateForFind.setUTCMonth(dateForFind.getUTCMonth() + 1);
  }
  dateForFind.setHours(-24);
  return dateForFind;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let offset = Math.floor((endDate - startDate) / (1000 * 60 * 60 * 24));
  ...
  return workingDays.map((date) =>
    Intl.DateTimeFormat('en-GB').format(date).split('/').join('-')
  );
}
